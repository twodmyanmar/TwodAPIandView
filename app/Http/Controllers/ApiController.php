...
class ApiController extends Controller
{
    ...
    public function __construct(protected TwoDigitCrawlerService $twoDigitCrawlerService)
    {
    }
    ...
    public function create(Request $request)
    {
        ...
        if (BlockedTwoDigit::isBlocked($data['number'])) {
            return response()->json([
                'message' => 'ဤကဏန်းသည် မရကဏန်းစာရင်းတွင် ပါသဖြင့် သိမ်းမရပါ။',
            ], 422);
        }
    ...
    public function update(Request $request, $id)
    {
        ...
        if (BlockedTwoDigit::isBlocked($data['number'])) {
            return response()->json([
                'message' => 'ဤကဏန်းသည် မရကဏန်းစာရင်းတွင် ပါသဖြင့် ပြင်မရပါ။',
            ], 422);
        }
    ...
    public function threedlive(ThreeDigitCrawlerService $threeDigitCrawlerService)
    {
        $data = $threeDigitCrawlerService->live();
        return response()->json($data);

    }

    public function fourdlive(FourDigitCrawlerService $fourDigitCrawlerService)
    {
        $data = $fourDigitCrawlerService->live();
        return response()->json($data);
    }
    ...
    public function calendar()
    {
        ...
        $calendarData = Results::whereBetween('recorded_at', [$firstDayOfMonth, $today])
                            ->whereIn('open_time', ['12:00', '12:01', '16:20', '16:30'])
                            ->get()
